...
def main():
    ...
    parser.add_argument('-o', type=str, help="Output file",
                        metavar='filename', default="../output/pyspark-blocking.csv")
    ...
    BETA = spark.sparkContext.broadcast(args.b)
    ...
    def mapper(element):
        ...
        if orig[0] == "A":
            s = int(math.floor(row / BETA.value))
            t = int(math.floor(col / BETA.value))

            for k in range(int(N.value / BETA.value)):
                key = (s, t, k)
                value = ('A', row, col, val)

                data.append((key, value))
        else:
            t = int(math.floor(row / BETA.value))
            k = int(math.floor(col / BETA.value))

            for s in range(int(N.value / BETA.value)):
                key = (s, t, k)
                value = ('B', row, col, val)
